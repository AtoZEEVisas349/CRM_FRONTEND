...
import { ChevronDown, ChevronUp, Users, User, UserCheck, Crown, Briefcase } from 'lucide-react';
import { useApi } from '../../context/ApiContext'; // Import the useApi hook
import SidebarToggle from "./SidebarToggle";

const Hierarchy = () => {
  const { fetchOrganizationHierarchyAPI, hierarchyLoading, organizationHierarchy } = useApi(); // Access API function and state
  ...
  const getRoleIcon = (role) => {
    switch (role) {
      case 'Admin': return <Crown className="oc-icon" />;
      case 'Manager': return <Users className="oc-icon" />;
      case 'Executive': return <UserCheck className="oc-icon" />;
      case 'TL': return <Briefcase className="oc-icon" />;
      case 'HR': return <User className="oc-icon" />;
      case 'ProcessPerson': return <User className="oc-icon" />;
      default: return <User className="oc-icon" />;
    }
  };

  const getRoleColor = (role) => {
    switch (role) {
      case 'Admin': return 'oc-gradient-admin';
      case 'Manager': return 'oc-gradient-manager';
      case 'Executive': return 'oc-gradient-executive';
      case 'TL': return 'oc-gradient-tl';
      case 'HR': return 'oc-gradient-hr';
      case 'ProcessPerson': return 'oc-gradient-process';
      default: return 'oc-gradient-default';
    }
  };
  ...
  const getRoleLevel = (role) => {
    switch (role) {
      ...
      case 'HR': return 2; // Changed from 3 to 2 to match Manager level
      case 'TL': return 3;
      case 'Executive': return 4;
      case 'ProcessPerson': return 4;
      default: return 0;
    }
  };
    ...
    >
      ...
      {node.children && node.children.length > 0 && (
        <div className="oc-toggle-button">
          ...
          >
            {expandedNodes.has(node.id) ? <ChevronUp className="oc-chevron" /> : <ChevronDown className="oc-chevron" />}
          </button>
        </div>
      )}
    </div>
      ...
      <div className="oc-children-wrapper">
        ...
        <div className="oc-children">
          {children.map((child, index) => (
            <div key={`${child.id}-${child.role}-${index}`} className="oc-child">
              <div className="oc-line-vertical-small"></div>
              <NodeBox node={child} isSelected={selectedNode && selectedNode.id === child.id && selectedNode.role === child.role} onClick={selectNode} />
              {expandedNodes.has(child.id) && renderChildren(child.children, child.id)}
            </div>
          ))}
        </div>
      </div>
  ...
  const renderGroupedChildren = (children, parentId) => {
    ...
    const combinedManagerHR = [...(groups['Manager'] || []), ...(groups['HR'] || [])];
      ...
      <div className="oc-grouped-wrapper">
        ...
        {combinedManagerHR.length > 0 && (
          <div key="ManagerHR" className="oc-role-group">
            ...
            {combinedManagerHR.length > 1 && <div className="oc-line-horizontal"></div>}
        ...
        {roleOrder.map(role => {
          ...
          return (
            <div key={role} className="oc-role-group">
              <div className="oc-role-title">{role}s ({groups[role].length}) - Level {getRoleLevel(role)}</div>
              {hasMultipleChildren && <div className="oc-line-horizontal"></div>}
              <div className="oc-role-children">
        ...
        <div className="oc-container">
          <div className="oc-header">
            ...
            <div className="oc-subheader">
              <div><strong>COMPANY:</strong> Your Company</div>
              <div><strong>COMPILED BY:</strong> System Admin</div>
              ...
              <div className="oc-chart">
                {orgData.hierarchy.map((rootNode) => (
                  <div key={rootNode.id} className="oc-root-node">
                    <NodeBox node={rootNode} isSelected={selectedNode && selectedNode.id === rootNode.id} onClick={selectNode} />
                    {expandedNodes.has(rootNode.id) && renderGroupedChildren(rootNode.children, rootNode.id)}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="oc-sidebar">
              <div className="oc-legend">
                ...
                <div className="oc-legend-items">
                  {['Admin', 'Manager', 'HR', 'TL', 'Executive', 'ProcessPerson'].map((role) => (
                    <div key={role} className="oc-legend-item">
                      <div className={`oc-legend-color ${getRoleColor(role)}`}></div>
                      <span>{role} (Level {getRoleLevel(role)})</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {selectedNode && (
                <div className="oc-details">
                  <h3 className="oc-details-title">Selected Employee Details</h3>
